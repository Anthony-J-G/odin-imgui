...
VERSION                 :: "1.92.7"
VERSION_NUM             :: 19270
TextureID_Invalid       :: cast(Texture_ID)0
...
Input_Text_Flag :: enum i32 {
    ...
    Ctrl_Enter_For_New_Line = 8, // In multi-line mode: validate with Enter, add new line with Ctrl+Enter (default is opposite: validate with Ctrl+Enter, add line with Enter). Note that Shift+Enter always enter a new line either way.
    ...
    Word_Wrap = 24, // InputTextMultiline(): word-wrap lines that are too long.
}
...
Tree_Node_Flag :: enum i32 {
    ...
    Allow_Overlap = 2, // Hit testing will allow subsequent widgets to overlap this one. Require previous frame HoveredId to match before being usable. Shortcut to calling SetNextItemAllowOverlap().
    ...
    Nav_Left_Jumps_To_Parent = 17, // Nav: left arrow moves back to parent. This is processed in TreePop() when there's an unfulfilled Left nav request remaining.
    Draw_Lines_None = 18, // No lines drawn
    Draw_Lines_Full = 19, // Horizontal lines to child nodes. Vertical line drawn down to TreePop() position: cover full contents. Faster (for large trees).
    Draw_Lines_To_Nodes = 20, // Horizontal lines to child nodes. Vertical line drawn down to bottom-most child node. Slower (for large trees).
}
...
Popup_Flag :: enum i32 {
    Mouse_Button_Left = 2, // For BeginPopupContext*(): open on Left Mouse release. Only one button allowed!
    Mouse_Button_Right = 3, // For BeginPopupContext*(): open on Right Mouse release. Only one button allowed! (default)
    Mouse_Button_Middle = 2, // For BeginPopupContext*(): open on Middle Mouse release. Only one button allowed!
...
Selectable_Flag :: enum i32 {
    ...
    Allow_Overlap = 4, // Hit testing will allow subsequent widgets to overlap this one. Require previous frame HoveredId to match before being usable. Shortcut to calling SetNextItemAllowOverlap().
    Highlight = 5, // Make the item be displayed as if it is hovered
    Select_On_Nav = 6, // Auto-select when moved into, unless Ctrl is held. Automatic when in a BeginMultiSelect() block.
}
...
Tab_Bar_Flag :: enum i32 {
    ...
    Fitting_Policy_Mixed = 7, // Shrink down tabs when they don't fit, until width is style.TabMinWidthShrink, then enable scrolling buttons.
    Fitting_Policy_Shrink = 8, // Shrink down tabs when they don't fit
    Fitting_Policy_Scroll = 9, // Enable scrolling buttons when tabs don't fit
}
...
HOVERED_FLAGS_RECT_ONLY :: Hovered_Flags{.Allow_When_Blocked_By_Popup, .Allow_When_Blocked_By_Active_Item, .Allow_When_Overlapped_By_Item, .Allow_When_Overlapped_By_Window, }
...
Drag_Drop_Flag :: enum i32 {
    ...
    Accept_Draw_As_Hovered = 13, // Accepting item will render as if hovered. Useful for e.g. a Button() used as a drop target.
}
...
Key :: enum i32 {
    ...
    Left_Shift = 528,
    Left_Alt = 529,
    Left_Super = 530, // Also see ImGuiMod_Ctrl, ImGuiMod_Shift, ImGuiMod_Alt, ImGuiMod_Super below!
    Right_Ctrl = 531,
    Right_Shift = 532,
    Right_Alt = 533,
    Right_Super = 534,
    ...
    Gamepad_Start = 632, // Menu        | +       | Options  |
    Gamepad_Back = 633, // View        | -       | Share    |
    Gamepad_Face_Left = 634, // X           | Y       | Square   | Toggle Menu. Hold for Windowing mode (Focus/Move/Resize windows)
    Gamepad_Face_Right = 635, // B           | A       | Circle   | Cancel / Close / Exit
    Gamepad_Face_Up = 636, // Y           | X       | Triangle | Open Context Menu
    Gamepad_Face_Down = 637, // A           | B       | Cross    | Activate / Open / Toggle. Hold for 0.60f to Activate in Text Input mode (e.g. wired to an on-screen keyboard).
    ...
    Gamepad_L1 = 642, // L Bumper    | L       | L1       | Tweak Slower / Focus Previous (in Windowing mode)
    Gamepad_R1 = 643, // R Bumper    | R       | R1       | Tweak Faster / Focus Next (in Windowing mode)
    Gamepad_L2 = 644, // L Trigger   | ZL      | L2       | [Analog]
    Gamepad_R2 = 645, // R Trigger   | ZR      | R2       | [Analog]
    Gamepad_L3 = 646, // L Stick     | L3      | L3       |
    Gamepad_R3 = 647, // R Stick     | R3      | R3       |
...
Config_Flag :: enum i32 {
    Nav_Enable_Keyboard = 0, // Master keyboard navigation enable flag. Enable full Tabbing + directional arrows + Space/Enter to activate. Note: some features such as basic Tabbing and CtrL+Tab are enabled by regardless of this flag (and may be disabled via other means, see #4828, #9218).
...
Backend_Flag :: enum i32 {
    ...
    Renderer_Has_Textures = 4, // Backend Renderer supports ImTextureData requests to create/update/destroy textures. This enables incremental texture updates and texture reloads. See https://github.com/ocornut/imgui/blob/master/docs/BACKENDS.md for instructions on how to upgrade your custom backend.
    Renderer_Has_Viewports = 10, // Backend Renderer supports multiple viewports.
    Platform_Has_Viewports = 11, // Backend Platform supports multiple viewports.
    Has_Mouse_Hovered_Viewport = 12, // Backend Platform supports calling io.AddMouseViewportEvent() with the viewport under the mouse. IF POSSIBLE, ignore viewports with the ImGuiViewportFlags_NoInputs flag (Win32 backend, GLFW 3.30+ backend can do this, SDL backend cannot). If this cannot be done, Dear ImGui needs to use a flawed heuristic to find the viewport under.
    Has_Parent_Viewport = 13, // Backend Platform supports honoring viewport->ParentViewport/ParentViewportId value, by applying the corresponding parent/child relation at the Platform level.
}
...
Col :: enum i32 {
    ...
    Input_Text_Cursor = 33, // InputText cursor/caret
    Tab_Hovered = 34, // Tab background, when hovered
    Tab = 35, // Tab background, when tab-bar is focused & tab is unselected
    Tab_Selected = 36, // Tab background, when tab-bar is focused & tab is selected
    Tab_Selected_Overline = 37, // Tab horizontal overline, when tab-bar is focused & tab is selected
    Tab_Dimmed = 38, // Tab background, when tab-bar is unfocused & tab is unselected
    Tab_Dimmed_Selected = 39, // Tab background, when tab-bar is unfocused & tab is selected
    Tab_Dimmed_Selected_Overline = 40, //..horizontal overline, when tab-bar is unfocused & tab is selected
    Docking_Preview = 41, // Preview overlay color when about to docking something
    Docking_Empty_Bg = 42, // Background color for empty node (e.g. CentralNode with no window docked into it)
    Plot_Lines = 43,
    Plot_Lines_Hovered = 44,
    Plot_Histogram = 45,
    Plot_Histogram_Hovered = 46,
    Table_Header_Bg = 47, // Table header background
    Table_Border_Strong = 48, // Table outer and header borders (prefer using Alpha=1.0 here)
    Table_Border_Light = 49, // Table inner borders (prefer using Alpha=1.0 here)
    Table_Row_Bg = 50, // Table row background (even rows)
    Table_Row_Bg_Alt = 51, // Table row background (odd rows)
    Text_Link = 52, // Hyperlink color
    Text_Selected_Bg = 53, // Selected text inside an InputText
    Tree_Lines = 54, // Tree node hierarchy outlines when using ImGuiTreeNodeFlags_DrawLines
    Drag_Drop_Target = 55, // Rectangle border highlighting a drop target
    Drag_Drop_Target_Bg = 56, // Rectangle background highlighting a drop target
    Unsaved_Marker = 57, // Unsaved Document marker (in window title and tabs)
    Nav_Cursor = 58, // Color of keyboard/gamepad navigation cursor/rectangle, when visible
    Nav_Windowing_Highlight = 59, // Highlight window when using Ctrl+Tab
    Nav_Windowing_Dim_Bg = 60, // Darken/colorize entire screen behind the Ctrl+Tab window list, when active
    Modal_Window_Dim_Bg = 61, // Darken/colorize entire screen behind a modal window, when one is active
}
...
Style_Var :: enum i32 {
    ...
    Scrollbar_Padding = 20, // float     ScrollbarPadding
    Grab_Min_Size = 21, // float     GrabMinSize
    Grab_Rounding = 22, // float     GrabRounding
    Image_Rounding = 23, // float     ImageRounding
    Image_Border_Size = 24, // float     ImageBorderSize
    Tab_Rounding = 25, // float     TabRounding
    Tab_Border_Size = 26, // float     TabBorderSize
    Tab_Min_Width_Base = 27, // float     TabMinWidthBase
    Tab_Min_Width_Shrink = 28, // float     TabMinWidthShrink
    Tab_Bar_Border_Size = 29, // float     TabBarBorderSize
    Tab_Bar_Overline_Size = 30, // float     TabBarOverlineSize
    Table_Angled_Headers_Angle = 31, // float     TableAngledHeadersAngle
    Table_Angled_Headers_Text_Align = 32, // ImVec2  TableAngledHeadersTextAlign
    Tree_Lines_Size = 33, // float     TreeLinesSize
    Tree_Lines_Rounding = 34, // float     TreeLinesRounding
    Button_Text_Align = 35, // ImVec2    ButtonTextAlign
    Selectable_Text_Align = 36, // ImVec2    SelectableTextAlign
    Separator_Size = 37, // float     SeparatorSize
    Separator_Text_Border_Size = 38, // float     SeparatorTextBorderSize
    Separator_Text_Align = 39, // ImVec2    SeparatorTextAlign
    Separator_Text_Padding = 40, // ImVec2    SeparatorTextPadding
    Docking_Separator_Size = 41, // float     DockingSeparatorSize
}
...
Button_Flag :: enum i32 {
    ...
    Allow_Overlap = 12, // Hit testing will allow subsequent widgets to overlap this one. Require previous frame HoveredId to match before being usable. Shortcut to calling SetNextItemAllowOverlap().
}
...
Color_Edit_Flag :: enum i32 {
    ...
    No_Drag_Drop = 9, //              // ColorEdit: disable drag and drop target/source. ColorButton: disable drag and drop source.
    No_Border = 10, //              // ColorButton: disable border (which is enforced by default)
    No_Color_Markers = 11, //              // ColorEdit: disable rendering R/G/B/A color marker. May also be disabled globally by setting style.ColorMarkerSize = 0.
    Alpha_Opaque = 12, //              // ColorEdit, ColorPicker, ColorButton: disable alpha in the preview,. Contrary to _NoAlpha it may still be edited when calling ColorEdit4()/ColorPicker4(). For ColorButton() this does the same as _NoAlpha.
    Alpha_No_Bg = 13, //              // ColorEdit, ColorPicker, ColorButton: disable rendering a checkerboard background behind transparent color.
    Alpha_Preview_Half = 14, //              // ColorEdit, ColorPicker, ColorButton: display half opaque / half transparent preview.
    Alpha_Bar = 18, //              // ColorEdit, ColorPicker: show vertical alpha bar/gradient in picker.
...
Slider_Flag :: enum i32 {
    ...
    Color_Markers = 12, // DragScalarN(), SliderScalarN(): Draw R/G/B/A color markers on each component.
}
...
Table_Flag :: enum i32 {
    ...
    No_Saved_Settings = 4, // Disable persisting columns order, width, visibility and sort settings in the .ini file.
    Context_Menu_In_Body = 5, // Right-click on columns body/contents will also display table context menu. By default it is available in TableHeadersRow().
...
TABLE_FLAGS_BORDERS :: Table_Flags{.Borders_Inner_V, .Borders_Inner_H, .Borders_Outer_V, .Borders_Outer_H, }
...
List_Clipper_Flags :: bit_set[List_Clipper_Flag; i32]
List_Clipper_Flag :: enum i32 {
    No_Set_Table_Row_Counters = 0, // [Internal] Disabled modifying table row counters. Avoid assumption that 1 clipper item == 1 table row.
}
...
Multi_Select_Flag :: enum i32 {
    ...
    Box_Select_No_Scroll = 8, // Disable scrolling when box-selecting and moving mouse near edges of scope.
    ...
    Select_On_Auto = 13, // Apply selection on mouse down when clicking on unselected item, on mouse up when clicking on selected item. (Default)
    Select_On_Click_Always = 14, // Apply selection on mouse down when clicking on any items. Prevents Drag and Drop from being used on multiple-selection, but allows e.g. BoxSelect to always reselect even when clicking inside an existing selection. (Excel style behavior)
    Select_On_Click_Release = 15, // Apply selection on mouse release when clicking an unselected item. Allow dragging an unselected item without altering selection.
    ...
    No_Select_On_Right_Click = 17, // Disable default right-click processing, which selects item on mouse down, and is designed for context-menus.
}
...
Texture_Status :: enum i32 {
    OK = 0,
    Destroyed = 1, // Backend destroyed the texture.
    Want_Create = 2, // Requesting backend to create the texture. Set status OK when done.
    Want_Updates = 3, // Requesting backend to update specific blocks of pixels (write to texture portions which have never been used before). Set status OK when done.
    Want_Destroy = 4, // Requesting backend to destroy the texture. Set status to Destroyed when done.
}
...
Font_Flags :: bit_set[Font_Flag; i32]
Font_Flag :: enum i32 {
    No_Load_Error = 1, // Disable throwing an error/assert when calling AddFontXXX() with missing file/data. Calling code is expected to check AddFontXXX() return value.
    No_Load_Glyphs = 2, // [Internal] Disable loading new glyphs.
    Lock_Baked_Sizes = 3, // [Internal] Disable loading new baked sizes, disable garbage collecting current ones. e.g. if you want to lock a font to a single size. Important: if you use this to preload given sizes, consider the possibility of multiple font density used on Retina display.
}
...
Font_Atlas_Rect_Id :: i32
Font_Atlas_Custom_Rect :: Font_Atlas_Rect

Draw_List_Shared_Data :: struct {}

Font_Atlas_Builder :: struct {}
...
Texture_Ref :: struct {
    _tex_data: ^Texture_Data, //      A texture, generally owned by a ImFontAtlas. Will convert to ImTextureID during render loop, after texture has been uploaded.
    _tex_id: Texture_ID, // _OR_ Low-level backend texture identifier, if already uploaded or created by user/app. Generally provided to e.g. ImGui::Image() calls.
}
...
Vector_Draw_List_Shared_Data_Ptr :: struct {
    size: i32,
    capacity: i32,
...
Vector_Texture_Data_Ptr :: struct {
    size: i32,
    capacity: i32,
    data: ^^Texture_Data,
}
...
Style :: struct {
    ...
    font_scale_dpi: f32, // Additional global scale factor from viewport/monitor contents scale. In docking branch: when io.ConfigDpiScaleFonts is enabled, this is automatically overwritten when changing monitor DPI.
    ...
    scrollbar_padding: f32, // Padding of scrollbar grab within its frame (same for both axes).
    ...
    image_rounding: f32, // Rounding of Image() calls.
    ...
    tree_lines_flags: Tree_Node_Flags, // Default way to draw lines connecting TreeNode hierarchy. ImGuiTreeNodeFlags_DrawLinesNone or ImGuiTreeNodeFlags_DrawLinesFull or ImGuiTreeNodeFlags_DrawLinesToNodes.
    ...
    drag_drop_target_rounding: f32, // Radius of the drag and drop target frame.
    drag_drop_target_border_size: f32, // Thickness of the drag and drop target border.
    drag_drop_target_padding: f32, // Size to expand the drag and drop target from actual target item size.
    color_marker_size: f32, // Size of R/G/B/A color markers for ColorEdit4() and for Drags/Sliders when using ImGuiSliderFlags_ColorMarkers.
    ...
    separator_size: f32, // Thickness of border in Separator()
    ...
    _main_scale: f32, // FIXME-WIP: Reference scale, as applied by ScaleAllSizes(). PLEASE DO NOT USE THIS FOR NOW.
...
IO :: struct {
    ...
    display_size: Vec2, // <unset>          // Main display size, in pixels (== GetMainViewport()->Size). May change every frame.
    display_framebuffer_scale: Vec2, // = (1, 1)         // Main display density. For retina display where window coordinates are different from framebuffer coordinates. This will affect font density + will end up in ImDrawData::FramebufferScale.
    ...
    config_viewports_platform_focus_sets_im_gui_focus: bool, //= true // When a platform window is focused (e.g. using Alt+Tab, clicking Platform Title Bar), apply corresponding focus on imgui windows (may clear focus/active id from imgui windows location in other platform windows). In principle this is better enabled but we provide an opt-out, because some Linux window managers tend to eagerly focus windows (e.g. on mouse hover, or even a simple window pos/size change).
    ...
    config_dpi_scale_viewports: bool, // = false          // [EXPERIMENTAL] Scale Dear ImGui and Platform Windows when Monitor DPI changes.
    ...
    config_input_text_enter_keep_active: bool, // = false          // [BETA] Pressing Enter will reactivate item and select all text (single-line only).
    ...
    keys_data: [KEY_NAMED_KEY_COUNT]Key_Data, // Key state for all known keys. MUST use 'key - ImGuiKey_NamedKey_BEGIN' as index. Use IsKeyXXX() functions to access this.
...
List_Clipper :: struct {
    ...
    user_index: i32, // Helper storage for user convenience/code. Optional, and otherwise unused if you don't use it.
    ...
    flags: List_Clipper_Flags, // [Internal] Flags, currently not yet well exposed.
    start_pos_y: f64, // [Internal] Cursor position at the time of Begin() or after table frozen rows are all processed
...
Selection_External_Storage :: struct {
    ...
    adapter_set_item_selected: proc "c" (self: ^Selection_External_Storage, idx: i32, selected: bool),
}
...
Draw_Cmd :: struct {
    ...
    tex_ref: Texture_Ref, // 16   // Reference to a font/texture atlas (where backend called ImTextureData::SetTexID()) or to a user-provided texture ID (via e.g. ImGui::Image() calls). Both will lead to a ImTextureID value.
...
Draw_List :: struct {
    ...
    _texture_stack: Vector_Texture_Ref, // [Internal]
...
Draw_Data :: struct {
    ...
    cmd_lists_count: i32, // == CmdLists.Size. (OBSOLETE: exists for legacy reasons). Number of ImDrawList* to render.
    ...
    framebuffer_scale: Vec2, // Amount of pixels for each unit of DisplaySize. Copied from viewport->FramebufferScale (== io.DisplayFramebufferScale for main viewport). Generally (1,1) on normal display, (2,2) on OSX with Retina display.
    owner_viewport: ^Viewport, // Viewport carrying the ImDrawData instance, might be of use to the renderer (generally not).
    textures: ^Vector_Texture_Data_Ptr, // List of textures to update. Most of the times the list is shared by all ImDrawData, has only 1 texture and it doesn't need any update. This almost always points to ImGui::GetPlatformIO().Textures[]. May be overridden or set to NULL if you want to manually update textures.
}
...
Texture_Data :: struct {
    unique_id: i32, // w    -   // [DEBUG] Sequential index to facilitate identifying a texture when debugging/printing. Unique per atlas.
    status: Texture_Status, // rw   rw  // ImTextureStatus_OK/_WantCreate/_WantUpdates/_WantDestroy. Always use SetStatus() to modify!
    backend_user_data: rawptr, // -    rw  // Convenience storage for backend. Some backends may have enough with TexID.
    tex_id: Texture_ID, // r    w   // Backend-specific texture identifier. Always use SetTexID() to modify! The identifier will stored in ImDrawCmd::GetTexID() and passed to backend's RenderDrawData function.
    format: Texture_Format, // w    r   // ImTextureFormat_RGBA32 (default) or ImTextureFormat_Alpha8
    width: i32, // w    r   // Texture width
    height: i32, // w    r   // Texture height
    bytes_per_pixel: i32, // w    r   // 4 or 1
    pixels: ^u8, // w    r   // Pointer to buffer holding 'Width*Height' pixels and 'Width*Height*BytesPerPixels' bytes.
    used_rect: Texture_Rect, // w    r   // Bounding box encompassing all past and queued Updates[].
    update_rect: Texture_Rect, // w    r   // Bounding box encompassing all queued Updates[].
    updates: Vector_Texture_Rect, // w    r   // Array of individual updates.
    unused_frames: i32, // w    r   // In order to facilitate handling Status==WantDestroy in some backend: this is a count successive frames where the texture was not used. Always >0 when Status==WantDestroy.
    ref_count: u16, // w    r   // Number of contexts using this texture. Used during backend shutdown.
    use_colors: bool, // w    r   // Tell whether our texture data is known to use colors (rather than just white + alpha).
...
Font_Config :: struct {
    ...
    oversample_h: i8, // 0 (2)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto == 1 or 2 depending on size. Note the difference between 2 and 3 is minimal. You can reduce this to 1 for large glyphs save memory. Read https://github.com/nothings/stb/blob/master/tests/oversample/README.md for details.
    oversample_v: i8, // 0 (1)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto == 1. This is not really useful as we don't use sub-pixel positions on the Y axis.
    ...
    size_pixels: f32, //          // Output size in pixels for rasterizer (more or less maps to the resulting font height).
    glyph_ranges: ^Wchar, // NULL     // *LEGACY* THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE. Pointer to a user-provided list of Unicode range (2 value per range, values are inclusive, zero-terminated list).
    glyph_exclude_ranges: ^Wchar, // NULL     // Pointer to a small user-provided list of Unicode ranges (2 value per range, values are inclusive, zero-terminated list). This is very close to GlyphRanges[] but designed to exclude ranges from a font source, when merging fonts with overlapping glyphs. Use "Input Glyphs Overlap Detection Tool" to find about your overlapping ranges.
    glyph_offset: Vec2, // 0, 0     // Offset (in pixels) all glyphs from this font input. Absolute value for default size, other sizes will scale this value.
    glyph_min_advance_x: f32, // 0        // Minimum AdvanceX for glyphs, set Min to align font icons, set both Min/Max to enforce mono-space font. Absolute value for default size, other sizes will scale this value.
    glyph_max_advance_x: f32, // FLT_MAX  // Maximum AdvanceX for glyphs
    glyph_extra_advance_x: f32, // 0        // Extra spacing (in pixels) between glyphs. Please contact us if you are using this. // FIXME-NEWATLAS: Intentionally unscaled
    font_no: u32, // 0        // Index of font within TTF/OTF file
    font_loader_flags: u32, // 0        // Settings for custom font builder. THIS IS BUILDER IMPLEMENTATION DEPENDENT. Leave as zero if unsure.
    ...
    rasterizer_density: f32, // 1.0f     // [LEGACY: this only makes sense when ImGuiBackendFlags_RendererHasTextures is not supported] DPI scale multiplier for rasterization. Not altering other font metrics: makes it easy to swap between e.g. a 100% and a 400% fonts for a zooming display, or handle Retina screen. IMPORTANT: If you change this it is expected that you increase/decrease font scale roughly to the inverse of this, otherwise quality may look lowered.
    extra_size_scale: f32, // 1.0f     // Extra rasterizer scale over SizePixels.
    flags: Font_Flags, // Font flags (don't use just yet, will be exposed in upcoming 1.92.X updates)
    dst_font: ^Font, // Target font (as we merging fonts, multiple ImFontConfig may target the same font)
    font_loader: ^Font_Loader, // Custom font backend for this source (default source is the one stored in ImFontAtlas)
    font_loader_data: rawptr, // Font loader opaque storage (per font config)
}
...
Font_Glyph :: struct {
    ...
    source_idx: u32, // Index of source in parent font
    codepoint: u32, // 0x0000..0x10FFFF
    advance_x: f32, // Horizontal distance to advance cursor/layout position.
    ...
    pack_id: i32, // [Internal] ImFontAtlasRectId value (FIXME: Cold data, could be moved elsewhere?)
}
...
Font_Atlas :: struct {
    ...
    tex_desired_format: Texture_Format, // Desired texture format (default to ImTextureFormat_RGBA32 but may be changed to ImTextureFormat_Alpha8).
    ...
    tex_min_width: i32, // Minimum desired texture width. Must be a power of two. Default to 512.
    tex_min_height: i32, // Minimum desired texture height. Must be a power of two. Default to 128.
    tex_max_width: i32, // Maximum desired texture width. Must be a power of two. Default to 8192.
    tex_max_height: i32, // Maximum desired texture height. Must be a power of two. Default to 8192.
    user_data: rawptr, // Store your own atlas related user-data (if e.g. you have multiple font atlas).
    tex_ref: Texture_Ref, // Latest texture identifier == TexData->GetTexRef().
    tex_data: ^Texture_Data, // Latest texture.
    tex_list: Vector_Texture_Data_Ptr, // Texture list (most often TexList.Size == 1). TexData is always == TexList.back(). DO NOT USE DIRECTLY, USE GetDrawData().Textures[]/GetPlatformIO().Textures[] instead!
    ...
    renderer_has_textures: bool, // Copy of (BackendFlags & ImGuiBackendFlags_RendererHasTextures) from supporting context.
    tex_is_built: bool, // Set when texture was built matching current font input. Mostly useful for legacy IsBuilt() call.
    ...
    tex_uv_scale: Vec2, // = (1.0f/TexData->TexWidth, 1.0f/TexData->TexHeight). May change as new texture gets created.
    tex_uv_white_pixel: Vec2, // Texture coordinates to a white pixel. May change as new texture gets created.
    ...
    tex_next_unique_id: i32, // Next value to be stored in TexData->UniqueID
    font_next_unique_id: i32, // Next value to be stored in ImFont->FontID
    draw_list_shared_datas: Vector_Draw_List_Shared_Data_Ptr, // List of users for this atlas. Typically one per Dear ImGui context.
    builder: ^Font_Atlas_Builder, // Opaque interface to our data that doesn't need to be public and may be discarded when rebuilding.
    font_loader: ^Font_Loader, // Font loader opaque interface (default to use FreeType when IMGUI_ENABLE_FREETYPE is defined, otherwise default to use stb_truetype). Use SetFontLoader() to change this at runtime.
    font_loader_name: cstring, // Font loader name (for display e.g. in About box) == FontLoader->Name
    font_loader_data: rawptr, // Font backend opaque storage
    font_loader_flags: u32, // Shared flags (for all fonts) for font loader. THIS IS BUILD IMPLEMENTATION DEPENDENT (e.g. Per-font override is also available in ImFontConfig).
    ref_count: i32, // Number of contexts using this atlas
...
Font_Baked :: struct {
    ...
    metrics_total_surface: u32, // 3  // out // Total surface in pixels to get an idea of the font rasterization/texture cost (not exact, we approximate the cost of padding between glyphs)
    want_destroy: u32, // 0  //     // Queued for destroy
    load_no_fallback: u32, // 0  //     // Disable loading fallback in lower-level calls.
    load_no_render_on_layout: u32, // 0  //     // Enable a two-steps mode where CalcTextSize() calls will load AdvanceX *without* rendering/packing glyphs. Only advantageous if you know that the glyph is unlikely to actually be rendered, otherwise it is slower because we'd do one query on the first CalcTextSize and one query on the first Draw.
    last_used_frame: i32, // 4  //     // Record of that time this was bounds
    baked_id: ID, // 4     //     // Unique ID for this baked storage
    owner_font: ^Font, // 4-8   // in  // Parent font
    font_loader_datas: rawptr, // 4-8   //     // Font loader opaque storage (per baked font * sources): single contiguous buffer allocated by imgui, passed to loader.
}
...
Font :: struct {
    last_baked: ^Font_Baked, // 4-8   // Cache last bound baked. NEVER USE DIRECTLY. Use GetFontBaked().
    owner_atlas: ^Font_Atlas, // 4-8   // What we have been loaded into.
    flags: Font_Flags, // 4     // Font flags.
    current_rasterizer_density: f32, // Current rasterizer density. This is a varying state of the font.
    font_id: ID, // Unique identifier for the font
    legacy_size: f32, // 4     // in  // Font size passed to AddFont(). Use for old code calling PushFont() expecting to use that size. (use ImGui::GetFontBaked() to get font baked at current bound size).
    sources: Vector_Font_Config_Ptr, // 16    // in  // List of sources. Pointers within OwnerAtlas->Sources[]
    ellipsis_char: Wchar, // 2-4   // out // Character used for ellipsis rendering ('...'). If you ever want to temporarily swap this for an alternative/dummy char, make sure to clear EllipsisAutoBake.
    fallback_char: Wchar, // 2-4   // out // Character used if a glyph isn't found (U+FFFD, '?')
    used8k_pages_map: [(UNICODE_CODEPOINT_MAX +1)/8192/8]u8, // 1 bytes if ImWchar=ImWchar16, 17 bytes if ImWchar==ImWchar32. Store 1-bit for each block of 8K codepoints that has one active glyph. This is mainly used to facilitate iterations across all used codepoints.
...
Viewport :: struct {
    ...
    framebuffer_scale: Vec2, // Density of the viewport for Retina display (always 1,1 on Windows, may be 2,2 etc on macOS/iOS). This will affect font rasterizer density.
    ...
    parent_viewport: ^Viewport, // (Advanced) Direct shortcut to ImGui::FindViewportByID(ParentViewportId). NULL: no parent.
...
Platform_IO :: struct {
    ...
    platform_set_ime_data_fn: proc "c" (ctx: ^Context, viewport: ^Viewport, data: ^Platforme_Data),
    platform_ime_user_data: rawptr,
    platform_locale_decimal_point: Wchar, // '.'
    renderer_texture_max_width: i32,
    renderer_texture_max_height: i32,
    ...
    platform_get_window_framebuffer_scale: proc "c" (vp: ^Viewport) -> Vec2,
    ...
    textures: Vector_Texture_Data_Ptr, // List of textures used by Dear ImGui (most often 1) + contents of external texture list is automatically appended into this.
...
Platforme_Data :: struct {
    ...
    input_pos: Vec2, // Position of input cursor (for IME).
    input_line_height: f32, // Line height (for IME).
...
foreign lib {
    ...
    texture_ref_get_tex_id :: proc(self: ^Texture_Ref) -> Texture_ID ---
    ...
    show_id_stack_tool_window :: proc() ---
    // create Stack Tool window. hover items with mouse to query information about the source of their unique ID.
    @(link_name = "ImGui_ShowIDStackToolWindowEx")
    show_id_stack_tool_window_ex :: proc(p_open: ^bool = nil) ---
    ...
    set_next_window_pos :: proc(pos: Vec2, cond: Cond = {}) ---
    // set next window position. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
    @(link_name = "ImGui_SetNextWindowPosEx")
    set_next_window_pos_ex :: proc(pos: Vec2, cond: Cond = {}, pivot: Vec2 = Vec2{0, 0}) ---
    ...
    push_font_float :: proc(font: ^Font, font_size_base_unscaled: f32) ---
    ...
    get_font_baked :: proc() -> ^Font_Baked ---
    ...
    pop_style_color :: proc() ---
    @(link_name = "ImGui_PopStyleColorEx")
    pop_style_color_ex :: proc(count: i32 = 1) ---
    ...
    pop_style_var_ex :: proc(count: i32 = 1) ---
    ...
    same_line_ex :: proc(offset_from_start_x: f32 = 0.0, spacing: f32 = -1.0) ---
    ...
    text_unformatted :: proc(text: cstring) ---
    // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
    @(link_name = "ImGui_TextUnformattedEx")
    text_unformatted_ex :: proc(text: cstring, text_end: cstring = nil) ---
    // formatted text
    ...
    // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
    @(link_name = "ImGui_TextColoredUnformatted")
    text_colored_unformatted :: proc(col: Vec4, text: cstring) ---
    // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
    @(link_name = "ImGui_TextDisabled")
    text_disabled :: proc(fmt: cstring, #c_vararg args: ..any) ---
    // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
    @(link_name = "ImGui_TextDisabledUnformatted")
    text_disabled_unformatted :: proc(text: cstring) ---
    // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
    @(link_name = "ImGui_TextWrapped")
    text_wrapped :: proc(fmt: cstring, #c_vararg args: ..any) ---
    // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
    @(link_name = "ImGui_TextWrappedUnformatted")
    text_wrapped_unformatted :: proc(text: cstring) ---
    // display text+label aligned the same way as value+label widgets
    @(link_name = "ImGui_LabelText")
    label_text :: proc(label: cstring, fmt: cstring, #c_vararg args: ..any) ---
    // display text+label aligned the same way as value+label widgets
    @(link_name = "ImGui_LabelTextUnformatted")
    label_text_unformatted :: proc(label: cstring, text: cstring) ---
    // shortcut for Bullet()+Text()
    @(link_name = "ImGui_BulletText")
    bullet_text :: proc(fmt: cstring, #c_vararg args: ..any) ---
    // shortcut for Bullet()+Text()
    @(link_name = "ImGui_BulletTextUnformatted")
    bullet_text_unformatted :: proc(text: cstring) ---
    ...
    button_ex :: proc(label: cstring, size: Vec2 = Vec2{0, 0}) -> bool ---
    ...
    text_link_open_url :: proc(label: cstring) -> bool ---
    // hyperlink text button, automatically open file/url when clicked
    @(link_name = "ImGui_TextLinkOpenURLEx")
    text_link_open_url_ex :: proc(label: cstring, url: cstring = nil) -> bool ---
    // Widgets: Images
    ...
    image :: proc(tex_ref: Texture_Ref, image_size: Vec2) ---
    @(link_name = "ImGui_ImageEx")
    image_ex :: proc(tex_ref: Texture_Ref, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}) ---
    // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
    @(link_name = "ImGui_ImageWithBg")
    image_with_bg :: proc(tex_ref: Texture_Ref, image_size: Vec2) ---
    @(link_name = "ImGui_ImageWithBgEx")
    image_with_bg_ex :: proc(tex_ref: Texture_Ref, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}, bg_col: Vec4 = Vec4{0, 0, 0, 0}, tint_col: Vec4 = Vec4{1, 1, 1, 1}) ---
    // Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
    @(link_name = "ImGui_ImageButton")
    image_button :: proc(str_id: cstring, tex_ref: Texture_Ref, image_size: Vec2) -> bool ---
    @(link_name = "ImGui_ImageButtonEx")
    image_button_ex :: proc(str_id: cstring, tex_ref: Texture_Ref, image_size: Vec2, uv0: Vec2 = Vec2{0, 0}, uv1: Vec2 = Vec2{1, 1}, bg_col: Vec4 = Vec4{0, 0, 0, 0}, tint_col: Vec4 = Vec4{1, 1, 1, 1}) -> bool ---
    ...
    combo_char :: proc(label: cstring, current_item: ^i32, items: [^]cstring, items_count: i32) -> bool ---
    @(link_name = "ImGui_ComboCharEx")
    combo_char_ex :: proc(label: cstring, current_item: ^i32, items: [^]cstring, items_count: i32, popup_max_height_in_items: i32 = -1) -> bool ---
    // Implied popup_max_height_in_items = -1
    @(link_name = "ImGui_Combo")
    combo :: proc(label: cstring, current_item: ^i32, items_separated_by_zeros: cstring) -> bool ---
    // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    @(link_name = "ImGui_ComboEx")
    combo_ex :: proc(label: cstring, current_item: ^i32, items_separated_by_zeros: cstring, popup_max_height_in_items: i32 = -1) -> bool ---
    // Implied popup_max_height_in_items = -1
    @(link_name = "ImGui_ComboCallback")
    combo_callback :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32) -> bool ---
    @(link_name = "ImGui_ComboCallbackEx")
    combo_callback_ex :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32, popup_max_height_in_items: i32 = -1) -> bool ---
    ...
    drag_float :: proc(label: cstring, v: ^f32) -> bool ---
    // If v_min >= v_max we have no bound
    @(link_name = "ImGui_DragFloatEx")
    drag_float_ex :: proc(label: cstring, v: ^f32, v_speed: f32 = 1.0, v_min: f32 = 0.0, v_max: f32 = 0.0, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
    @(link_name = "ImGui_DragFloat2")
    drag_float2 :: proc(label: cstring, v: ^[2]f32) -> bool ---
    @(link_name = "ImGui_DragFloat2Ex")
    drag_float2ex :: proc(label: cstring, v: ^[2]f32, v_speed: f32 = 1.0, v_min: f32 = 0.0, v_max: f32 = 0.0, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
    @(link_name = "ImGui_DragFloat3")
    drag_float3 :: proc(label: cstring, v: ^[3]f32) -> bool ---
    @(link_name = "ImGui_DragFloat3Ex")
    drag_float3ex :: proc(label: cstring, v: ^[3]f32, v_speed: f32 = 1.0, v_min: f32 = 0.0, v_max: f32 = 0.0, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
    @(link_name = "ImGui_DragFloat4")
    drag_float4 :: proc(label: cstring, v: ^[4]f32) -> bool ---
    @(link_name = "ImGui_DragFloat4Ex")
    drag_float4ex :: proc(label: cstring, v: ^[4]f32, v_speed: f32 = 1.0, v_min: f32 = 0.0, v_max: f32 = 0.0, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", format_max = NULL, flags = 0
    @(link_name = "ImGui_DragFloatRange2")
    drag_float_range2 :: proc(label: cstring, v_current_min: ^f32, v_current_max: ^f32) -> bool ---
    @(link_name = "ImGui_DragFloatRange2Ex")
    drag_float_range2ex :: proc(label: cstring, v_current_min: ^f32, v_current_max: ^f32, v_speed: f32 = 1.0, v_min: f32 = 0.0, v_max: f32 = 0.0, format: cstring = "%.3f", format_max: cstring = nil, flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
    @(link_name = "ImGui_DragInt")
    drag_int :: proc(label: cstring, v: ^i32) -> bool ---
    // If v_min >= v_max we have no bound
    @(link_name = "ImGui_DragIntEx")
    drag_int_ex :: proc(label: cstring, v: ^i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
    @(link_name = "ImGui_DragInt2")
    drag_int2 :: proc(label: cstring, v: ^[2]i32) -> bool ---
    @(link_name = "ImGui_DragInt2Ex")
    drag_int2ex :: proc(label: cstring, v: ^[2]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
    @(link_name = "ImGui_DragInt3")
    drag_int3 :: proc(label: cstring, v: ^[3]i32) -> bool ---
    @(link_name = "ImGui_DragInt3Ex")
    drag_int3ex :: proc(label: cstring, v: ^[3]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
    @(link_name = "ImGui_DragInt4")
    drag_int4 :: proc(label: cstring, v: ^[4]i32) -> bool ---
    @(link_name = "ImGui_DragInt4Ex")
    drag_int4ex :: proc(label: cstring, v: ^[4]i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", format_max = NULL, flags = 0
    @(link_name = "ImGui_DragIntRange2")
    drag_int_range2 :: proc(label: cstring, v_current_min: ^i32, v_current_max: ^i32) -> bool ---
    @(link_name = "ImGui_DragIntRange2Ex")
    drag_int_range2ex :: proc(label: cstring, v_current_min: ^i32, v_current_max: ^i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", format_max: cstring = nil, flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
    @(link_name = "ImGui_DragScalar")
    drag_scalar :: proc(label: cstring, data_type: Data_Type, p_data: rawptr) -> bool ---
    @(link_name = "ImGui_DragScalarEx")
    drag_scalar_ex :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, v_speed: f32 = 1.0, p_min: rawptr = nil, p_max: rawptr = nil, format: cstring = nil, flags: Slider_Flags = {}) -> bool ---
    // Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
    @(link_name = "ImGui_DragScalarN")
    drag_scalar_n :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, components: i32) -> bool ---
    @(link_name = "ImGui_DragScalarNEx")
    drag_scalar_n_ex :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, components: i32, v_speed: f32 = 1.0, p_min: rawptr = nil, p_max: rawptr = nil, format: cstring = nil, flags: Slider_Flags = {}) -> bool ---
    ...
    slider_float :: proc(label: cstring, v: ^f32, v_min: f32, v_max: f32) -> bool ---
    // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
    @(link_name = "ImGui_SliderFloatEx")
    slider_float_ex :: proc(label: cstring, v: ^f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%.3f", flags = 0
    @(link_name = "ImGui_SliderFloat2")
    slider_float2 :: proc(label: cstring, v: ^[2]f32, v_min: f32, v_max: f32) -> bool ---
    @(link_name = "ImGui_SliderFloat2Ex")
    slider_float2ex :: proc(label: cstring, v: ^[2]f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%.3f", flags = 0
    @(link_name = "ImGui_SliderFloat3")
    slider_float3 :: proc(label: cstring, v: ^[3]f32, v_min: f32, v_max: f32) -> bool ---
    @(link_name = "ImGui_SliderFloat3Ex")
    slider_float3ex :: proc(label: cstring, v: ^[3]f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%.3f", flags = 0
    @(link_name = "ImGui_SliderFloat4")
    slider_float4 :: proc(label: cstring, v: ^[4]f32, v_min: f32, v_max: f32) -> bool ---
    @(link_name = "ImGui_SliderFloat4Ex")
    slider_float4ex :: proc(label: cstring, v: ^[4]f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied v_degrees_min = -360.0f, v_degrees_max = +360.0f, format = "%.0f deg", flags = 0
    @(link_name = "ImGui_SliderAngle")
    slider_angle :: proc(label: cstring, v_rad: ^f32) -> bool ---
    @(link_name = "ImGui_SliderAngleEx")
    slider_angle_ex :: proc(label: cstring, v_rad: ^f32, v_degrees_min: f32 = -360.0, v_degrees_max: f32 = +360.0, format: cstring = "%.0f deg", flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%d", flags = 0
    @(link_name = "ImGui_SliderInt")
    slider_int :: proc(label: cstring, v: ^i32, v_min: i32, v_max: i32) -> bool ---
    @(link_name = "ImGui_SliderIntEx")
    slider_int_ex :: proc(label: cstring, v: ^i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%d", flags = 0
    @(link_name = "ImGui_SliderInt2")
    slider_int2 :: proc(label: cstring, v: ^[2]i32, v_min: i32, v_max: i32) -> bool ---
    @(link_name = "ImGui_SliderInt2Ex")
    slider_int2ex :: proc(label: cstring, v: ^[2]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%d", flags = 0
    @(link_name = "ImGui_SliderInt3")
    slider_int3 :: proc(label: cstring, v: ^[3]i32, v_min: i32, v_max: i32) -> bool ---
    @(link_name = "ImGui_SliderInt3Ex")
    slider_int3ex :: proc(label: cstring, v: ^[3]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%d", flags = 0
    @(link_name = "ImGui_SliderInt4")
    slider_int4 :: proc(label: cstring, v: ^[4]i32, v_min: i32, v_max: i32) -> bool ---
    @(link_name = "ImGui_SliderInt4Ex")
    slider_int4ex :: proc(label: cstring, v: ^[4]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied format = NULL, flags = 0
    @(link_name = "ImGui_SliderScalar")
    slider_scalar :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, p_min: rawptr, p_max: rawptr) -> bool ---
    @(link_name = "ImGui_SliderScalarEx")
    slider_scalar_ex :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: Slider_Flags = {}) -> bool ---
    // Implied format = NULL, flags = 0
    @(link_name = "ImGui_SliderScalarN")
    slider_scalar_n :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, components: i32, p_min: rawptr, p_max: rawptr) -> bool ---
    @(link_name = "ImGui_SliderScalarNEx")
    slider_scalar_n_ex :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, components: i32, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%.3f", flags = 0
    @(link_name = "ImGui_VSliderFloat")
    v_slider_float :: proc(label: cstring, size: Vec2, v: ^f32, v_min: f32, v_max: f32) -> bool ---
    @(link_name = "ImGui_VSliderFloatEx")
    v_slider_float_ex :: proc(label: cstring, size: Vec2, v: ^f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: Slider_Flags = {}) -> bool ---
    // Implied format = "%d", flags = 0
    @(link_name = "ImGui_VSliderInt")
    v_slider_int :: proc(label: cstring, size: Vec2, v: ^i32, v_min: i32, v_max: i32) -> bool ---
    @(link_name = "ImGui_VSliderIntEx")
    v_slider_int_ex :: proc(label: cstring, size: Vec2, v: ^i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: Slider_Flags = {}) -> bool ---
    // Implied format = NULL, flags = 0
    @(link_name = "ImGui_VSliderScalar")
    v_slider_scalar :: proc(label: cstring, size: Vec2, data_type: Data_Type, p_data: rawptr, p_min: rawptr, p_max: rawptr) -> bool ---
    @(link_name = "ImGui_VSliderScalarEx")
    v_slider_scalar_ex :: proc(label: cstring, size: Vec2, data_type: Data_Type, p_data: rawptr, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: Slider_Flags = {}) -> bool ---
    ...
    input_text :: proc(label: cstring, buf: cstring, buf_size: uint, flags: Input_Text_Flags = {}) -> bool ---
    @(link_name = "ImGui_InputTextEx")
    input_text_ex :: proc(label: cstring, buf: cstring, buf_size: uint, flags: Input_Text_Flags = {}, callback: Input_Text_Callback = nil, user_data: rawptr = nil) -> bool ---
    // Implied size = ImVec2(0, 0), flags = 0, callback = NULL, user_data = NULL
    @(link_name = "ImGui_InputTextMultiline")
    input_text_multiline :: proc(label: cstring, buf: cstring, buf_size: uint) -> bool ---
    @(link_name = "ImGui_InputTextMultilineEx")
    input_text_multiline_ex :: proc(label: cstring, buf: cstring, buf_size: uint, size: Vec2 = Vec2{0, 0}, flags: Input_Text_Flags = {}, callback: Input_Text_Callback = nil, user_data: rawptr = nil) -> bool ---
    // Implied callback = NULL, user_data = NULL
    @(link_name = "ImGui_InputTextWithHint")
    input_text_with_hint :: proc(label: cstring, hint: cstring, buf: cstring, buf_size: uint, flags: Input_Text_Flags = {}) -> bool ---
    @(link_name = "ImGui_InputTextWithHintEx")
    input_text_with_hint_ex :: proc(label: cstring, hint: cstring, buf: cstring, buf_size: uint, flags: Input_Text_Flags = {}, callback: Input_Text_Callback = nil, user_data: rawptr = nil) -> bool ---
    ...
    input_float_ex :: proc(label: cstring, v: ^f32, step: f32 = 0.0, step_fast: f32 = 0.0, format: cstring = "%.3f", flags: Input_Text_Flags = {}) -> bool ---
    // Implied format = "%.3f", flags = 0
    @(link_name = "ImGui_InputFloat2")
    input_float2 :: proc(label: cstring, v: ^[2]f32) -> bool ---
    @(link_name = "ImGui_InputFloat2Ex")
    input_float2ex :: proc(label: cstring, v: ^[2]f32, format: cstring = "%.3f", flags: Input_Text_Flags = {}) -> bool ---
    // Implied format = "%.3f", flags = 0
    @(link_name = "ImGui_InputFloat3")
    input_float3 :: proc(label: cstring, v: ^[3]f32) -> bool ---
    @(link_name = "ImGui_InputFloat3Ex")
    input_float3ex :: proc(label: cstring, v: ^[3]f32, format: cstring = "%.3f", flags: Input_Text_Flags = {}) -> bool ---
    // Implied format = "%.3f", flags = 0
    @(link_name = "ImGui_InputFloat4")
    input_float4 :: proc(label: cstring, v: ^[4]f32) -> bool ---
    @(link_name = "ImGui_InputFloat4Ex")
    input_float4ex :: proc(label: cstring, v: ^[4]f32, format: cstring = "%.3f", flags: Input_Text_Flags = {}) -> bool ---
    // Implied step = 1, step_fast = 100, flags = 0
    @(link_name = "ImGui_InputInt")
    input_int :: proc(label: cstring, v: ^i32) -> bool ---
    @(link_name = "ImGui_InputIntEx")
    input_int_ex :: proc(label: cstring, v: ^i32, step: i32 = 1, step_fast: i32 = 100, flags: Input_Text_Flags = {}) -> bool ---
    ...
    input_double :: proc(label: cstring, v: ^f64) -> bool ---
    @(link_name = "ImGui_InputDoubleEx")
    input_double_ex :: proc(label: cstring, v: ^f64, step: f64 = 0.0, step_fast: f64 = 0.0, format: cstring = "%.6f", flags: Input_Text_Flags = {}) -> bool ---
    // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
    @(link_name = "ImGui_InputScalar")
    input_scalar :: proc(label: cstring, data_type: Data_Type, p_data: rawptr) -> bool ---
    @(link_name = "ImGui_InputScalarEx")
    input_scalar_ex :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, p_step: rawptr = nil, p_step_fast: rawptr = nil, format: cstring = nil, flags: Input_Text_Flags = {}) -> bool ---
    // Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
    @(link_name = "ImGui_InputScalarN")
    input_scalar_n :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, components: i32) -> bool ---
    @(link_name = "ImGui_InputScalarNEx")
    input_scalar_n_ex :: proc(label: cstring, data_type: Data_Type, p_data: rawptr, components: i32, p_step: rawptr = nil, p_step_fast: rawptr = nil, format: cstring = nil, flags: Input_Text_Flags = {}) -> bool ---
    ...
    color_button :: proc(desc_id: cstring, col: Vec4, flags: Color_Edit_Flags = {}) -> bool ---
    // display a color square/button, hover for details, return true when pressed.
    @(link_name = "ImGui_ColorButtonEx")
    color_button_ex :: proc(desc_id: cstring, col: Vec4, flags: Color_Edit_Flags = {}, size: Vec2 = Vec2{0, 0}) -> bool ---
    ...
    // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
    @(link_name = "ImGui_TreeNodeStrUnformatted")
    tree_node_str_unformatted :: proc(str_id: cstring, text: cstring) -> bool ---
    // "
    @(link_name = "ImGui_TreeNodePtr")
    tree_node_ptr :: proc(ptr_id: rawptr, fmt: cstring, #c_vararg args: ..any) -> bool ---
    // "
    @(link_name = "ImGui_TreeNodePtrUnformatted")
    tree_node_ptr_unformatted :: proc(ptr_id: rawptr, text: cstring) -> bool ---
    ...
    @(link_name = "ImGui_TreeNodeExStrUnformatted")
    tree_node_ex_str_unformatted :: proc(str_id: cstring, flags: Tree_Node_Flags, text: cstring) -> bool ---
    ...
    @(link_name = "ImGui_TreeNodeExPtrUnformatted")
    tree_node_ex_ptr_unformatted :: proc(ptr_id: rawptr, flags: Tree_Node_Flags, text: cstring) -> bool ---
    ...
    tree_node_get_open :: proc(storage_id: ID) -> bool ---
    // Widgets: Selectables
    // - A selectable highlights when hovered, and can display another color when selected.
    // - Neighbors selectable extend their highlight bounds in order to leave no gap between them. This is so a series of selected Selectable appear contiguous.
    // Implied selected = false, flags = 0, size = ImVec2(0, 0)
    @(link_name = "ImGui_Selectable")
    selectable :: proc(label: cstring) -> bool ---
    // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
    @(link_name = "ImGui_SelectableEx")
    selectable_ex :: proc(label: cstring, selected: bool = false, flags: Selectable_Flags = {}, size: Vec2 = Vec2{0, 0}) -> bool ---
    // Implied size = ImVec2(0, 0)
    @(link_name = "ImGui_SelectableBoolPtr")
    selectable_bool_ptr :: proc(label: cstring, p_selected: ^bool, flags: Selectable_Flags = {}) -> bool ---
    // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
    @(link_name = "ImGui_SelectableBoolPtrEx")
    selectable_bool_ptr_ex :: proc(label: cstring, p_selected: ^bool, flags: Selectable_Flags = {}, size: Vec2 = Vec2{0, 0}) -> bool ---
    ...
    begin_multi_select :: proc(flags: Multi_Select_Flags) -> ^Multi_Select_IO ---
    @(link_name = "ImGui_BeginMultiSelectEx")
    begin_multi_select_ex :: proc(flags: Multi_Select_Flags, selection_size: i32 = -1, items_count: i32 = -1) -> ^Multi_Select_IO ---
    ...
    list_box_callback :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32) -> bool ---
    @(link_name = "ImGui_ListBoxCallbackEx")
    list_box_callback_ex :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32, height_in_items: i32 = -1) -> bool ---
    ...
    plot_lines :: proc(label: cstring, values: ^f32, values_count: i32) ---
    @(link_name = "ImGui_PlotLinesEx")
    plot_lines_ex :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = Vec2{0, 0}, stride: i32 = size_of(f32)) ---
    ...
    plot_lines_callback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32) ---
    @(link_name = "ImGui_PlotLinesCallbackEx")
    plot_lines_callback_ex :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = Vec2{0, 0}) ---
    // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0), stride = sizeof(float)
    @(link_name = "ImGui_PlotHistogram")
    plot_histogram :: proc(label: cstring, values: ^f32, values_count: i32) ---
    @(link_name = "ImGui_PlotHistogramEx")
    plot_histogram_ex :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = Vec2{0, 0}, stride: i32 = size_of(f32)) ---
    // Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0)
    @(link_name = "ImGui_PlotHistogramCallback")
    plot_histogram_callback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32) ---
    @(link_name = "ImGui_PlotHistogramCallbackEx")
    plot_histogram_callback_ex :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = Vec2{0, 0}) ---
    // Widgets: Menus
    ...
    begin_menu_ex :: proc(label: cstring, enabled: bool = true) -> bool ---
    ...
    menu_item_ex :: proc(label: cstring, shortcut: cstring = nil, selected: bool = false, enabled: bool = true) -> bool ---
    ...
    // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
    @(link_name = "ImGui_SetTooltipUnformatted")
    set_tooltip_unformatted :: proc(text: cstring) ---
    ...
    // set a text-only tooltip if preceding item was hovered. override any previous call to SetTooltip().
    @(link_name = "ImGui_SetItemTooltipUnformatted")
    set_item_tooltip_unformatted :: proc(text: cstring) ---
    // Popups, Modals
    ...
    open_popup_on_item_click :: proc(str_id: cstring = nil, popup_flags: Popup_Flags = {}) ---
    ...
    begin_popup_context_item :: proc() -> bool ---
    // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
    @(link_name = "ImGui_BeginPopupContextItemEx")
    begin_popup_context_item_ex :: proc(str_id: cstring = nil, popup_flags: Popup_Flags = {}) -> bool ---
    // Implied str_id = NULL, popup_flags = 0
    @(link_name = "ImGui_BeginPopupContextWindow")
    begin_popup_context_window :: proc() -> bool ---
    // open+begin popup when clicked on current window.
    @(link_name = "ImGui_BeginPopupContextWindowEx")
    begin_popup_context_window_ex :: proc(str_id: cstring = nil, popup_flags: Popup_Flags = {}) -> bool ---
    // Implied str_id = NULL, popup_flags = 0
    @(link_name = "ImGui_BeginPopupContextVoid")
    begin_popup_context_void :: proc() -> bool ---
    // open+begin popup when clicked in void (where there are no windows).
    @(link_name = "ImGui_BeginPopupContextVoidEx")
    begin_popup_context_void_ex :: proc(str_id: cstring = nil, popup_flags: Popup_Flags = {}) -> bool ---
    ...
    begin_table :: proc(str_id: cstring, columns: i32, flags: Table_Flags = {}) -> bool ---
    @(link_name = "ImGui_BeginTableEx")
    begin_table_ex :: proc(str_id: cstring, columns: i32, flags: Table_Flags = {}, outer_size: Vec2 = Vec2{0.0, 0.0}, inner_width: f32 = 0.0) -> bool ---
    ...
    table_next_row_ex :: proc(row_flags: Table_Row_Flags = {}, min_row_height: f32 = 0.0) ---
    ...
    table_setup_column :: proc(label: cstring, flags: Table_Column_Flags = {}) ---
    @(link_name = "ImGui_TableSetupColumnEx")
    table_setup_column_ex :: proc(label: cstring, flags: Table_Column_Flags = {}, init_width_or_weight: f32 = 0.0, user_id: ID = {}) ---
    ...
    columns_ex :: proc(count: i32 = 1, id: cstring = nil, borders: bool = true) ---
    ...
    dock_space :: proc(dockspace_id: ID) -> ID ---
    @(link_name = "ImGui_DockSpaceEx")
    dock_space_ex :: proc(dockspace_id: ID, size: Vec2 = Vec2{0, 0}, flags: Dock_Node_Flags = {}, window_class: ^Window_Class = nil) -> ID ---
    // Implied dockspace_id = 0, viewport = NULL, flags = 0, window_class = NULL
    @(link_name = "ImGui_DockSpaceOverViewport")
    dock_space_over_viewport :: proc() -> ID ---
    @(link_name = "ImGui_DockSpaceOverViewportEx")
    dock_space_over_viewport_ex :: proc(dockspace_id: ID = {}, viewport: ^Viewport = nil, flags: Dock_Node_Flags = {}, window_class: ^Window_Class = nil) -> ID ---
    ...
    // pass text data straight to log (without being displayed)
    @(link_name = "ImGui_LogTextUnformatted")
    log_text_unformatted :: proc(text: cstring) ---
    // Drag and Drop
    ...
    set_keyboard_focus_here :: proc() ---
    // focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.
    @(link_name = "ImGui_SetKeyboardFocusHereEx")
    set_keyboard_focus_here_ex :: proc(offset: i32 = 0) ---
    ...
    is_item_clicked :: proc() -> bool ---
    // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) && IsItemHovered()Important. (**) this is NOT equivalent to the behavior of e.g. Button(). Read comments in function definition.
    @(link_name = "ImGui_IsItemClickedEx")
    is_item_clicked_ex :: proc(mouse_button: Mouse_Button = {}) -> bool ---
    ...
    get_background_draw_list_ex :: proc(viewport: ^Viewport = nil) -> ^Draw_List ---
    // Implied viewport = NULL
    ...
    get_foreground_draw_list_ex :: proc(viewport: ^Viewport = nil) -> ^Draw_List ---
    ...
    calc_text_size :: proc(text: cstring) -> Vec2 ---
    @(link_name = "ImGui_CalcTextSizeEx")
    calc_text_size_ex :: proc(text: cstring, text_end: cstring = nil, hide_text_after_double_hash: bool = false, wrap_width: f32 = -1.0) -> Vec2 ---
    // Color Utilities
    ...
    is_key_pressed_ex :: proc(key: Key, repeat: bool = true) -> bool ---
    ...
    is_mouse_clicked :: proc(button: Mouse_Button) -> bool ---
    // did mouse button clicked? (went from !Down to Down). Same as GetMouseClickedCount() == 1.
    @(link_name = "ImGui_IsMouseClickedEx")
    is_mouse_clicked_ex :: proc(button: Mouse_Button, repeat: bool = false) -> bool ---
    ...
    is_mouse_hovering_rect :: proc(r_min: Vec2, r_max: Vec2) -> bool ---
    // is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
    @(link_name = "ImGui_IsMouseHoveringRectEx")
    is_mouse_hovering_rect_ex :: proc(r_min: Vec2, r_max: Vec2, clip: bool = true) -> bool ---
    ...
    reset_mouse_drag_delta :: proc() ---
    //
    @(link_name = "ImGui_ResetMouseDragDeltaEx")
    reset_mouse_drag_delta_ex :: proc(button: Mouse_Button = {}) ---
    ...
    // Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
    @(link_name = "ImGui_DebugLogUnformatted")
    debug_log_unformatted :: proc(text: cstring) ---
    // Memory Allocators
    ...
    render_platform_windows_default :: proc() ---
    // call in main loop. will call RenderWindow/SwapBuffers platform functions for each secondary viewport which doesn't have the ImGuiViewportFlags_Minimized flag set. May be reimplemented by user for custom rendering needs.
    @(link_name = "ImGui_RenderPlatformWindowsDefaultEx")
    render_platform_windows_default_ex :: proc(platform_render_arg: rawptr = nil, renderer_render_arg: rawptr = nil) ---
    ...
    find_viewport_by_id :: proc(viewport_id: ID) -> ^Viewport ---
    ...
    platform_io_set_platform_get_window_work_area_insets :: proc(getWindowWorkAreaInsetsFunc: proc "c" (vp: ^Viewport, result: ^Vec4)) ---
    ...
    platform_io_set_platform_get_window_framebuffer_scale :: proc(getWindowFramebufferScaleFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) ---
    // Set ImGuiPlatformIO::Platform_GetWindowPos in a C-compatible mannner
    @(link_name = "ImGuiPlatformIO_SetPlatform_GetWindowPos")
    platform_io_set_platform_get_window_pos :: proc(getWindowPosFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) ---
    // Set ImGuiPlatformIO::Platform_GetWindowSize in a C-compatible mannner
    @(link_name = "ImGuiPlatformIO_SetPlatform_GetWindowSize")
    platform_io_set_platform_get_window_size :: proc(getWindowSizeFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) ---
    ...
    io_set_key_event_native_data :: proc(self: ^IO, key: Key, native_keycode: i32, native_scancode: i32) ---
    // [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with native indices + specify native keycode, scancode.
    @(link_name = "ImGuiIO_SetKeyEventNativeDataEx")
    io_set_key_event_native_data_ex :: proc(self: ^IO, key: Key, native_keycode: i32, native_scancode: i32, native_legacy_index: i32 = -1) ---
    ...
    input_text_callback_data_set_selection :: proc(self: ^Input_Text_Callback_Data, s: i32, e: i32) ---
    ...
    text_filter_gui_text_range_split :: proc(self: ^Text_Filter_Gui_Text_Range, separator: cstring, out: ^Vector_Gui_Text_Range) ---
    ...
    draw_list_push_texture :: proc(self: ^Draw_List, tex_ref: Texture_Ref) ---
    @(link_name = "ImDrawList_PopTexture")
    draw_list_pop_texture :: proc(self: ^Draw_List) ---
    ...
    draw_list_add_line :: proc(self: ^Draw_List, p1: Vec2, p2: Vec2, col: u32) ---
    @(link_name = "ImDrawList_AddLineEx")
    draw_list_add_line_ex :: proc(self: ^Draw_List, p1: Vec2, p2: Vec2, col: u32, thickness: f32 = 1.0) ---
    // Implied rounding = 0.0f, flags = 0, thickness = 1.0f
    @(link_name = "ImDrawList_AddRect")
    draw_list_add_rect :: proc(self: ^Draw_List, p_min: Vec2, p_max: Vec2, col: u32) ---
    // a: upper-left, b: lower-right (== upper-left + size)
    @(link_name = "ImDrawList_AddRectEx")
    draw_list_add_rect_ex :: proc(self: ^Draw_List, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0.0, flags: Draw_Flags = {}, thickness: f32 = 1.0) ---
    // Implied rounding = 0.0f, flags = 0
    @(link_name = "ImDrawList_AddRectFilled")
    draw_list_add_rect_filled :: proc(self: ^Draw_List, p_min: Vec2, p_max: Vec2, col: u32) ---
    // a: upper-left, b: lower-right (== upper-left + size)
    @(link_name = "ImDrawList_AddRectFilledEx")
    draw_list_add_rect_filled_ex :: proc(self: ^Draw_List, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0.0, flags: Draw_Flags = {}) ---
    ...
    draw_list_add_quad :: proc(self: ^Draw_List, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, col: u32) ---
    @(link_name = "ImDrawList_AddQuadEx")
    draw_list_add_quad_ex :: proc(self: ^Draw_List, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, col: u32, thickness: f32 = 1.0) ---
    ...
    draw_list_add_triangle :: proc(self: ^Draw_List, p1: Vec2, p2: Vec2, p3: Vec2, col: u32) ---
    @(link_name = "ImDrawList_AddTriangleEx")
    draw_list_add_triangle_ex :: proc(self: ^Draw_List, p1: Vec2, p2: Vec2, p3: Vec2, col: u32, thickness: f32 = 1.0) ---
    ...
    draw_list_add_circle :: proc(self: ^Draw_List, center: Vec2, radius: f32, col: u32) ---
    @(link_name = "ImDrawList_AddCircleEx")
    draw_list_add_circle_ex :: proc(self: ^Draw_List, center: Vec2, radius: f32, col: u32, num_segments: i32 = 0, thickness: f32 = 1.0) ---
    ...
    draw_list_add_ngon :: proc(self: ^Draw_List, center: Vec2, radius: f32, col: u32, num_segments: i32) ---
    @(link_name = "ImDrawList_AddNgonEx")
    draw_list_add_ngon_ex :: proc(self: ^Draw_List, center: Vec2, radius: f32, col: u32, num_segments: i32, thickness: f32 = 1.0) ---
    ...
    draw_list_add_ellipse :: proc(self: ^Draw_List, center: Vec2, radius: Vec2, col: u32) ---
    @(link_name = "ImDrawList_AddEllipseEx")
    draw_list_add_ellipse_ex :: proc(self: ^Draw_List, center: Vec2, radius: Vec2, col: u32, rot: f32 = 0.0, num_segments: i32 = 0, thickness: f32 = 1.0) ---
    // Implied rot = 0.0f, num_segments = 0
    @(link_name = "ImDrawList_AddEllipseFilled")
    draw_list_add_ellipse_filled :: proc(self: ^Draw_List, center: Vec2, radius: Vec2, col: u32) ---
    @(link_name = "ImDrawList_AddEllipseFilledEx")
    draw_list_add_ellipse_filled_ex :: proc(self: ^Draw_List, center: Vec2, radius: Vec2, col: u32, rot: f32 = 0.0, num_segments: i32 = 0) ---
    // Implied text_end = NULL
    @(link_name = "ImDrawList_AddText")
    draw_list_add_text :: proc(self: ^Draw_List, pos: Vec2, col: u32, text_begin: cstring) ---
    @(link_name = "ImDrawList_AddTextEx")
    draw_list_add_text_ex :: proc(self: ^Draw_List, pos: Vec2, col: u32, text_begin: cstring, text_end: cstring = nil) ---
    // Implied text_end = NULL, wrap_width = 0.0f, cpu_fine_clip_rect = NULL
    @(link_name = "ImDrawList_AddTextImFontPtr")
    draw_list_add_text_font_ptr :: proc(self: ^Draw_List, font: ^Font, font_size: f32, pos: Vec2, col: u32, text_begin: cstring) ---
    @(link_name = "ImDrawList_AddTextImFontPtrEx")
    draw_list_add_text_font_ptr_ex :: proc(self: ^Draw_List, font: ^Font, font_size: f32, pos: Vec2, col: u32, text_begin: cstring, text_end: cstring = nil, wrap_width: f32 = 0.0, cpu_fine_clip_rect: ^Vec4 = nil) ---
    ...
    im_draw_list_add_image :: proc(self: ^Draw_List, tex_ref: Texture_Ref, p_min: Vec2, p_max: Vec2) ---
    @(link_name = "ImDrawList_AddImageEx")
    im_draw_list_add_image_ex :: proc(self: ^Draw_List, tex_ref: Texture_Ref, p_min: Vec2, p_max: Vec2, uv_min: Vec2 = Vec2{0, 0}, uv_max: Vec2 = Vec2{1, 1}, col: u32 = 0xff_ff_ff_ff) ---
    // Implied uv1 = ImVec2(0, 0), uv2 = ImVec2(1, 0), uv3 = ImVec2(1, 1), uv4 = ImVec2(0, 1), col = IM_COL32_WHITE
    @(link_name = "ImDrawList_AddImageQuad")
    im_draw_list_add_image_quad :: proc(self: ^Draw_List, tex_ref: Texture_Ref, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2) ---
    @(link_name = "ImDrawList_AddImageQuadEx")
    im_draw_list_add_image_quad_ex :: proc(self: ^Draw_List, tex_ref: Texture_Ref, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, uv1: Vec2 = Vec2{0, 0}, uv2: Vec2 = Vec2{1, 0}, uv3: Vec2 = Vec2{1, 1}, uv4: Vec2 = Vec2{0, 1}, col: u32 = 0xff_ff_ff_ff) ---
    @(link_name = "ImDrawList_AddImageRounded")
    im_draw_list_add_image_rounded :: proc(self: ^Draw_List, tex_ref: Texture_Ref, p_min: Vec2, p_max: Vec2, uv_min: Vec2, uv_max: Vec2, col: u32, rounding: f32, flags: Draw_Flags = {}) ---
    ...
    draw_list_path_elliptical_arc_to :: proc(self: ^Draw_List, center: Vec2, radius: Vec2, rot: f32, a_min: f32, a_max: f32) ---
    // Ellipse
    @(link_name = "ImDrawList_PathEllipticalArcToEx")
    draw_list_path_elliptical_arc_to_ex :: proc(self: ^Draw_List, center: Vec2, radius: Vec2, rot: f32, a_min: f32, a_max: f32, num_segments: i32 = 0) ---
    // Cubic Bezier (4 control points)
    ...
    draw_list_add_callback :: proc(self: ^Draw_List, callback: Draw_Callback, userdata: rawptr) ---
    @(link_name = "ImDrawList_AddCallbackEx")
    draw_list_add_callback_ex :: proc(self: ^Draw_List, callback: Draw_Callback, userdata: rawptr, userdata_size: uint = {}) ---
    ...
    draw_list_set_draw_list_shared_data :: proc(self: ^Draw_List, data: ^Draw_List_Shared_Data) ---
    ...
    draw_list_on_changed_texture :: proc(self: ^Draw_List) ---
    ...
    draw_list_set_texture :: proc(self: ^Draw_List, tex_ref: Texture_Ref) ---
    ...
    texture_data_create :: proc(self: ^Texture_Data, format: Texture_Format, w: i32, h: i32) ---
    @(link_name = "ImTextureData_DestroyPixels")
    texture_data_destroy_pixels :: proc(self: ^Texture_Data) ---
    @(link_name = "ImTextureData_GetPixels")
    texture_data_get_pixels :: proc(self: ^Texture_Data) -> rawptr ---
    @(link_name = "ImTextureData_GetPixelsAt")
    texture_data_get_pixels_at :: proc(self: ^Texture_Data, x: i32, y: i32) -> rawptr ---
    @(link_name = "ImTextureData_GetSizeInBytes")
    texture_data_get_size_in_bytes :: proc(self: ^Texture_Data) -> i32 ---
    @(link_name = "ImTextureData_GetPitch")
    texture_data_get_pitch :: proc(self: ^Texture_Data) -> i32 ---
    @(link_name = "ImTextureData_GetTexRef")
    texture_data_get_tex_ref :: proc(self: ^Texture_Data) -> Texture_Ref ---
    @(link_name = "ImTextureData_GetTexID")
    texture_data_get_tex_id :: proc(self: ^Texture_Data) -> Texture_ID ---
    ...
    texture_data_set_tex_id :: proc(self: ^Texture_Data, tex_id: Texture_ID) ---
    @(link_name = "ImTextureData_SetStatus")
    texture_data_set_status :: proc(self: ^Texture_Data, status: Texture_Status) ---
    ...
    font_atlas_add_font_default_vector :: proc(self: ^Font_Atlas, font_cfg: ^Font_Config = nil) -> ^Font ---
    // Embedded classic pixel-clean font. Recommended at Size 13px with no scaling.
    @(link_name = "ImFontAtlas_AddFontDefaultBitmap")
    font_atlas_add_font_default_bitmap :: proc(self: ^Font_Atlas, font_cfg: ^Font_Config = nil) -> ^Font ---
    @(link_name = "ImFontAtlas_AddFontFromFileTTF")
    font_atlas_add_font_from_file_ttf :: proc(self: ^Font_Atlas, filename: cstring, size_pixels: f32 = 0.0, font_cfg: ^Font_Config = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
    // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after destruction of the atlas. Set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your data and it won't be freed.
    @(link_name = "ImFontAtlas_AddFontFromMemoryTTF")
    font_atlas_add_font_from_memory_ttf :: proc(self: ^Font_Atlas, font_data: rawptr, font_data_size: i32, size_pixels: f32 = 0.0, font_cfg: ^Font_Config = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
    // 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp.
    @(link_name = "ImFontAtlas_AddFontFromMemoryCompressedTTF")
    font_atlas_add_font_from_memory_compressed_ttf :: proc(self: ^Font_Atlas, compressed_font_data: rawptr, compressed_font_data_size: i32, size_pixels: f32 = 0.0, font_cfg: ^Font_Config = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
    // 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 parameter.
    @(link_name = "ImFontAtlas_AddFontFromMemoryCompressedBase85TTF")
    font_atlas_add_font_from_memory_compressed_base85ttf :: proc(self: ^Font_Atlas, compressed_font_data_base85: cstring, size_pixels: f32 = 0.0, font_cfg: ^Font_Config = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
    @(link_name = "ImFontAtlas_RemoveFont")
    font_atlas_remove_font :: proc(self: ^Font_Atlas, font: ^Font) ---
    ...
    font_atlas_compact_cache :: proc(self: ^Font_Atlas) ---
    // Change font loader at runtime.
    @(link_name = "ImFontAtlas_SetFontLoader")
    font_atlas_set_font_loader :: proc(self: ^Font_Atlas, font_loader: ^Font_Loader) ---
    ...
    font_atlas_add_custom_rect :: proc(self: ^Font_Atlas, width: i32, height: i32, out_r: ^Font_Atlas_Rect = nil) -> Font_Atlas_Rect_Id ---
    ...
    font_atlas_remove_custom_rect :: proc(self: ^Font_Atlas, id: Font_Atlas_Rect_Id) ---
    ...
    font_atlas_get_custom_rect :: proc(self: ^Font_Atlas, id: Font_Atlas_Rect_Id, out_r: ^Font_Atlas_Rect) -> bool ---
    @(link_name = "ImFontBaked_ClearOutputData")
    font_baked_clear_output_data :: proc(self: ^Font_Baked) ---
    // Return U+FFFD glyph if requested glyph doesn't exists.
    @(link_name = "ImFontBaked_FindGlyph")
    font_baked_find_glyph :: proc(self: ^Font_Baked, c: Wchar) -> ^Font_Glyph ---
    // Return NULL if glyph doesn't exist
    @(link_name = "ImFontBaked_FindGlyphNoFallback")
    font_baked_find_glyph_no_fallback :: proc(self: ^Font_Baked, c: Wchar) -> ^Font_Glyph ---
    @(link_name = "ImFontBaked_GetCharAdvance")
    font_baked_get_char_advance :: proc(self: ^Font_Baked, c: Wchar) -> f32 ---
    @(link_name = "ImFontBaked_IsGlyphLoaded")
    font_baked_is_glyph_loaded :: proc(self: ^Font_Baked, c: Wchar) -> bool ---
    @(link_name = "ImFont_IsGlyphInFont")
    font_is_glyph_in_font :: proc(self: ^Font, c: Wchar) -> bool ---
    ...
    font_get_font_baked :: proc(self: ^Font, font_size: f32) -> ^Font_Baked ---
    // Get or create baked data for given size
    @(link_name = "ImFont_GetFontBakedEx")
    font_get_font_baked_ex :: proc(self: ^Font, font_size: f32, density: f32 = -1.0) -> ^Font_Baked ---
    // Implied text_end = NULL, out_remaining = NULL
    @(link_name = "ImFont_CalcTextSizeA")
    font_calc_text_size_a :: proc(self: ^Font, size: f32, max_width: f32, wrap_width: f32, text_begin: cstring) -> Vec2 ---
    @(link_name = "ImFont_CalcTextSizeAEx")
    font_calc_text_size_a_ex :: proc(self: ^Font, size: f32, max_width: f32, wrap_width: f32, text_begin: cstring, text_end: cstring = nil, out_remaining: ^cstring = nil) -> Vec2 ---
    @(link_name = "ImFont_CalcWordWrapPosition")
    font_calc_word_wrap_position :: proc(self: ^Font, size: f32, text: cstring, text_end: cstring, wrap_width: f32) -> cstring ---
    ...
    @(link_name = "ImFont_RenderCharEx")
    font_render_char_ex :: proc(self: ^Font, draw_list: ^Draw_List, size: f32, pos: Vec2, col: u32, c: Wchar, cpu_fine_clip: ^Vec4 = nil) ---
    @(link_name = "ImFont_RenderText")
    font_render_text :: proc(self: ^Font, draw_list: ^Draw_List, size: f32, pos: Vec2, col: u32, clip_rect: Vec4, text_begin: cstring, text_end: cstring, wrap_width: f32 = 0.0, flags: Draw_Text_Flags = {}) ---
    // [Internal] Don't use!
    ...
    font_add_remap_char :: proc(self: ^Font, from_codepoint: Wchar, to_codepoint: Wchar) ---
    ...
    viewport_get_debug_name :: proc(self: ^Viewport) -> cstring ---
    ...
    platform_io_clear_platform_handlers :: proc(self: ^Platform_IO) ---
    ...
    platform_io_clear_renderer_handlers :: proc(self: ^Platform_IO) ---
}